...
public class JsonMLUtil {
	...
	private static final Logger LOG = LoggerFactory.getLogger(JsonMLUtil.class);
	...
	public static String getJsonDataForGetTimeSeriesData(DatapointsResponse dpr, JsonMapper jsonMapper) {
		...
		for (int q = 0; q < dpr.getTags().size(); q++) {
			...
			for (Object objList : dprValuesList) {
				...
				SimpleDateFormat DATE_FORMAT = new SimpleDateFormat(MLConstants.DATE_FOMAT_GETDATA);
