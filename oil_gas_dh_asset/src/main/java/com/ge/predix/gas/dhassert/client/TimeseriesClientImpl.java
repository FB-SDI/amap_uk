...
package com.ge.predix.gas.dhassert.client;
...
import com.ge.predix.gas.dhassert.api.TimeSeriesAPIV1;
import com.ge.predix.gas.dhassert.config.ITimeseriesConfig;
...
public class TimeseriesClientImpl implements TimeseriesClient {
	...
	public void createConnectionToTimeseriesWebsocket(WebSocketAdapter messageListener) {
		try {
			...
			WebSocketAdapter listener = new WebSocketAdapter()
            {
                ...
                {
                    System.out.println("Success from Predix Websocket Server message=" + message);
                }
            };
	...
	public void postDataToTimeseriesWebsocket(DatapointsIngestion datapointsIngestion) {
		try {
			...
			System.out.println(this.wsClient.toString());

			this.wsClient.postTextWSData(request);
			
			
			System.out.println(" sending posted ----");
		} catch (IOException | WebSocketException e) {
			System.out.println("-----Exception while posting -----");
